...
export async function setupCommands(chatClient, twitchEventManager) {
  try {
    ...
    // Set up Claude with twitchEventManager
    const { claude, handler: claudeHandler } = setupClaude(chatClient, twitchEventManager);
    commands.set('claude', claude);
    ...
    const registerCommand = (name, handler, source) => {
      if (!commands.has(name)) {
        ...
        commands.set(name, wrappedHandler);
    ...
    logger.debug(`Registered commands: ${Array.from(commands.keys()).join(', ')}`);
